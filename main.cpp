...
int main(int argc, char *argv[])
{
    ...
    po::path file_name_boost(data_file);
    double model_resolution = computeCloudResolution (cloud);
    radius = model_resolution * 10;
    ...
    pcl::PointIndicesConstPtr point_indices = iss_detector.getKeypointsIndices();
    if(point_indices == NULL)
        return 0;    
    std::string save_file_name = output_folder + "/" + file_name_boost.stem().string() + "_iss.ply";
    //pcl::io::savePLYFileASCII(caca,*model_keypoints);
    std::cout<<"saving to "<<save_file_name<<std::endl;
    pcl::io::savePLYFile(save_file_name,*model_keypoints);
    ...
    std::vector<int> evaluation_points = point_indices->indices;
...
*/
    ...
    //save_file_name = temp_token.substr(0, found2);
    //save_file_name = output_folder + save_file_name;
    save_file_name = output_folder + "/" + file_name_boost.stem().string() + "_smooth_features.csv";
    std::string features_file_name = file_name_boost.stem().string() + "_tf_features.csv";
